...
def fetch_fred_candidates(codes, name):
    """Try several FRED tickers and return the first valid series."""
    cutoff = TODAY - dt.timedelta(days=LOOKBACK_DAYS * 2)
    for code in codes:
        try:
            df = fdr.DataReader(f"FRED:{code}", cutoff.strftime("%Y-%m-%d"))
            if df is None or df.empty:
                continue
            col = df.columns[0]
            ...
            s.index = pd.to_datetime(s.index).date
            s = s.rename(name)
            print(f"  fred {name} via {code}: {len(s)} rows")
            return s
        except Exception as e:
            print(f"  [info] fred {name} {code}: {e}")
            continue
    return pd.Series(dtype=float, name=name)


def fetch_stooq_close(symbol, name=None, days=LOOKBACK_DAYS * 2):
    """Daily close series from Stooq CSV endpoint."""
    url = f"https://stooq.com/q/d/l/?s={symbol.lower()}&i=d"
    headers = {"User-Agent": "Mozilla/5.0", "Referer": "https://stooq.com/"}
    try:
        r = requests.get(url, headers=headers, timeout=20)
        r.raise_for_status()
        if len(r.text) < 20 or 'Date,Open,High,Low,Close' not in r.text:
            return pd.Series(dtype=float, name=name or symbol)
        from io import StringIO
        ...
        if not s.empty:
            print(f"  stooq {name or symbol}: {len(s)} rows, latest {float(s.iloc[-1]):.2f}")
        return s
    except Exception as e:
...
def fetch_global_liquidity_proxy():
    """
    Global liquidity proxy (A안): normalize major central-bank balance-sheet series to Base 100 and average.
    This is a proxy, not a dollar-summed exact measure.
    """
    series_map = {
        'fed': fetch_fred_candidates(['WALCL'], 'fed'),
        'ecb': fetch_fred_candidates(['ECBASSETSW', 'ECBASSETS'], 'ecb'),
        'boj': fetch_fred_candidates(['JPNASSETS', 'BOJASSETS', 'JPNCBBS'], 'boj'),
        'pboc': fetch_fred_candidates(['CHNASSETS', 'PBOCASSETS'], 'pboc'),
    }
    valid = [s for s in series_map.values() if s is not None and not s.empty]
    if not valid:
        print('  [warn] global liquidity proxy: all sources failed')
        return pd.Series(dtype=float, name='global_liquidity')
    ...
    for c in df.columns:
        ...
        if base <= 0:
            continue
        norm_cols.append((df[c] / base) * 100)
    if not norm_cols:
        return pd.Series(dtype=float, name='global_liquidity')
    out = pd.concat(norm_cols, axis=1).mean(axis=1).rename('global_liquidity')
    print(f"  global liquidity proxy: {len(out)} rows, latest {float(out.iloc[-1]):.2f}")
    return out


def fetch_usdjpy_series():
    """USDJPY daily series from Stooq first, Yahoo/FDR fallback."""
    s = fetch_stooq_close('usdjpy', 'usdjpy')
    if not s.empty:
        return s
    for sym in ['USDJPY', 'JPYUSD', 'USD/JPY']:
        try:
            s2 = fetch_fdr(sym, 'usdjpy', days=LOOKBACK_DAYS * 2)
            if not s2.empty:
                return s2
        except Exception:
            continue
...
def update_data():
    ...
    global_liquidity = safe("global_liquidity", fetch_global_liquidity_proxy, default=pd.Series(dtype=float, name='global_liquidity'))
    usdjpy = safe("usdjpy", fetch_usdjpy_series, default=pd.Series(dtype=float, name='usdjpy'))
    ...
    extras = {
        ...
        "global_liquidity": global_liquidity,     # Series (Base 100 proxy)
        "usdjpy": usdjpy,                         # Series
    }
...
<div id="pane-us" class="pane">
  ...
  <div class="chart"><div id="c_us_m7" style="height:700px;"></div></div>
  <div class="chart"><div id="c_us_liquidity" style="height:420px;"></div></div>
</div>
...
(function() {{
  const id = 'c_us_liquidity';
  const el = document.getElementById(id);
  if (!el) return;
  if (!D.liq_dates || D.liq_dates.length === 0 || !hasValues(D.liq_vals) || !hasValues(D.usdjpy_vals)) {{
    showEmpty(id);
    return;
  }}
  try {{
    Plotly.newPlot(id, [
      {{
        x: D.liq_dates, y: D.liq_vals, type: 'scatter', mode: 'lines', name: '글로벌 유동성 proxy',
        connectgaps: true, line: {{color: '#185FA5', width: 2.8}}
      }},
      {{
        x: D.usdjpy_dates, y: D.usdjpy_vals, type: 'scatter', mode: 'lines', name: '달러-엔', yaxis: 'y2',
        connectgaps: true, line: {{color: '#9CA3AF', width: 2.6}}
      }}
    ], Object.assign({{}}, base, {{
      ...
      legend: {{orientation: 'h', y: -0.15, x: 0, xanchor: 'left'}},
      margin: {{t: 40, r: 60, b: 55, l: 60}}
    }}), {{displayModeBar: false, responsive: true}});
  }} catch (e) {{ console.error('liq plot:', e); showEmpty(id); }}
}})();
