...
KST = dt.timezone(dt.timedelta(hours=9))# -*- coding: utf-8 -*-
# bittu monitor v5
# Trend outlook + fetch error fixes.
...
TODAY = dt.datetime.now(KST).date()
LOOKBACK_DAYS = 400
RUNNING_ON_GITHUB_ACTIONS = os.environ.get("GITHUB_ACTIONS", "").strip().lower() == "true"
ENABLE_BLOCKED_WEB_SOURCES = (
    not RUNNING_ON_GITHUB_ACTIONS
    and os.environ.get("MONITOR_ENABLE_BLOCKED_WEB_SOURCES", "").strip().lower() in {
        "1", "true", "yes", "on"
    }
)
